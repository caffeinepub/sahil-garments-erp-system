...
export default function InventoryModule({ userProfile }: InventoryModuleProps) {
  ...
  const lowStockProducts = products.filter(
    (item) => Number(item.stockLevel) < 10,
  );
  ...
  const handleScanResult = (barcode: string) => {
    ...
    if (product) {
      ...
      const _stockInfo = inventoryItem
        ? `${Number(inventoryItem.quantity)} units`
        : "No inventory record";
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    all inventory data including products, stock levels, and
                    transaction history from the system.
                    <br />
                    <br />
                    <strong className="text-destructive">Warning:</strong> This
                    is an admin-only operation that will remove all{" "}
                    {products.length} product(s) and their associated data from
                    the database.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
      ...
      >
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Point your camera at a product barcode to look up stock
              information
            </DialogDescription>
          </DialogHeader>
