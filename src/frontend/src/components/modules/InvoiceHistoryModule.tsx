...
export default function InvoiceHistoryModule({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: formatCurrency is a stable pure function defined outside render
  const filteredAndSortedInvoices = useMemo(() => {
    ...
    if (selectedCustomer !== "all") {
      filtered = filtered.filter(
        (inv) => inv.customerId.toString() === selectedCustomer,
      );
    }
    ...
    if (dateRange) {
      filtered = filtered.filter(
        (inv) => inv.created >= dateRange.start && inv.created <= dateRange.end,
      );
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case "invoiceNumber":
          ...
          break;
        case "customerName":
          comparison = getCustomerName(a.customerId).localeCompare(
  ...
  const handleClearAllInvoices = async () => {
    try {
      ...
      toast.error(
        error.message || "Failed to clear invoices. Please try again.",
      );
    }
  };
  ...
  const exportToPDF = async () => {
    toast.info(
      "PDF export functionality would generate a comprehensive report of filtered invoices",
    );
  ...
  const exportToCSV = () => {
    try {
      ...
      link.setAttribute(
        "download",
        `invoice-history-${new Date().toISOString().split("T")[0]}.csv`,
      );
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <Card className="max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Invoice History is restricted to Admin and Sales roles only.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            View and manage all generated invoices with advanced filtering
          </p>
        </div>
                ...
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription className="space-y-2">
                    <p className="font-semibold text-destructive">
                      ...
                      {invoices.length !== 1 ? "s" : ""} from the system.
                    </p>
                    <p>
                      This operation cannot be undone. All invoice records will
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Filter invoices by date, customer, and status
              </CardDescription>
            </div>
            {(searchQuery ||
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Click on any invoice to preview and download
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={previewDialogOpen} onOpenChange={setPreviewDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Invoice Preview - SG-{selectedInvoice?.invoiceId.toString()}
            </DialogTitle>
            <DialogDescription>
              Preview and download invoice for{" "}
