...
import { useListInventory, useListOrders } from "../../hooks/useQueries";
...
export default function ReportsModule({ userProfile }: ReportsModuleProps) {
    ...
    return (
      <div className="space-y-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            You don't have permission to access reports. Only Accountants and
            Admins can use this feature.
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  const todayOrders = orders.filter(
    (order) => Number(order.created) >= todayStart,
  );

  const thisMonth =
    new Date(today.getFullYear(), today.getMonth(), 1).getTime() * 1000000;
  const monthlyOrders = orders.filter(
    (order) => Number(order.created) >= thisMonth,
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            View business reports and system alerts
          </p>
        </div>
      </div>
