...
import { T as InvoiceStatus, type UserProfile } from "../../backend";
import {
  useCreateInvoice,
  useGetInvoice,
  useListCustomers,
  useListProducts,
  useStockAdjustInvoice,
...
export default function InvoiceModule({
  ...
  const handleAddItem = () => {
    ...
    const product = products.find(
      (p) => p.productId.toString() === currentProductId,
    );
    if (!product) {
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      const { subtotal: _subtotal, tax, total } = calculateTotals();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Generate invoices for customers with automatic stock adjustment
        </p>
      </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          Stock will be automatically adjusted when the invoice is created. Make
          sure to verify quantities before proceeding.
        </AlertDescription>
      </Alert>
                  ...
                  >
                    ...
                    <SelectContent>
                      {products.map((product) => (
                        ...
                        >
                          {product.name} - ₹{Number(product.price)} (Stock:{" "}
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                {invoiceItems.map((item, index) => {
                  ...
                  return (
                    <div
                      // biome-ignore lint/suspicious/noArrayIndexKey: invoice items list uses index as stable key within session
                      key={index}
