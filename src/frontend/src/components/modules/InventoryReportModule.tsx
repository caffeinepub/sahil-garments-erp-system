...
export default function InventoryReportModule() {
  ...
  const [barcodeCanvases, setBarcodeCanvases] = useState<
    Map<number, HTMLCanvasElement>
  >(new Map());
  const [failedProducts, setFailedProducts] = useState<Set<number>>(new Set());
  const [librariesLoaded, setLibrariesLoaded] = useState(false);
  const _canvasRefs = useRef<Map<number, HTMLCanvasElement>>(new Map());

  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional — only re-run when products list changes; generatingBarcodes/librariesLoaded are guards, not triggers
  useEffect(() => {
  ...
  // Cleanup on unmount
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional — cleanup only runs on unmount
  useEffect(() => {
    return () => {
      for (const canvas of barcodeCanvases.values()) {
        canvas.remove();
      }
    };
  }, []);
  ...
  const generateBarcodeForProduct = (
    product: Product,
    ...
    try {
      ...
      if (!validation.valid) {
        console.error(
          `Product ${product.name} validation failed:`,
          validation.error,
        );
        return null;
      }
      ...
      try {
        window.JsBarcode(canvas, product.barcode, {
          ...
          valid: (valid: boolean) => {
            if (!valid) {
              console.error(
                `Invalid barcode format for product ${product.name}: ${product.barcode}`,
              );
            }
          },
        });
        ...
        if (canvas.width === 0 || canvas.height === 0) {
          console.error(
            `Canvas has zero dimensions for product ${product.name}`,
          );
          return null;
        }
        ...
        console.error(
          `JsBarcode generation failed for product ${product.name}:`,
          barcodeError,
        );
        return null;
      }
      ...
      console.error(
        `Error generating barcode for product ${product.name}:`,
        error,
      );
      return null;
    }
  ...
  const generateAllBarcodes = async () => {
    ...
    try {
      ...
      if (failed.size > 0) {
        toast.warning(
          `Generated ${newCanvases.size} barcodes. ${failed.size} failed.`,
        );
      } else {
      ...
      toast.error(
        "Failed to load barcode generation library. Please refresh the page.",
      );
    } finally {
  ...
  const exportIndividualBarcodePDF = async (product: Product) => {
    ...
    try {
      ...
      pdf.text("Wholesale & Retail Clothing Store", pageWidth / 2, 19, {
        align: "center",
      });
      ...
      pdf.text(
        `Stock Level: ${Number(product.stockLevel)} units`,
        20,
        detailsY + 16,
      );
      pdf.text(
        `Location: ${product.warehouse} - ${product.rack} - ${product.shelf}`,
        20,
        detailsY + 24,
      );
      pdf.text(
      ...
      pdf.text(
        "© 2025 Sahil Garments. All rights reserved.",
        pageWidth / 2,
        pageHeight - 10,
        { align: "center" },
      );
      ...
      pdf.save(
        `${product.name.replace(/[^a-z0-9]/gi, "_")}-${product.barcode}-barcode.pdf`,
      );
  ...
  const exportAllBarcodesPDF = async () => {
    ...
    const validProducts = products.filter((p) =>
      barcodeCanvases.has(Number(p.productId)),
    );
    ...
    try {
      ...
      validProducts.forEach((product, index) => {
        ...
        try {
          ...
          if (isFirstPage && index === 0) {
            ...
            pdf.setFontSize(16);
            pdf.setFont("helvetica", "bold");
            pdf.text("Sahil Garments - Inventory Barcodes", pageWidth / 2, 12, {
              align: "center",
            });
            currentY = 30;
          }
          ...
          if (imgData && imgData !== "data:,") {
            ...
            pdf.text(
              `Stock: ${Number(product.stockLevel)} | Location: ${product.warehouse}-${product.rack}-${product.shelf}`,
              pageWidth / 2,
              detailsY + 5,
              { align: "center" },
            );
      ...
      toast.success(
        `All barcodes exported as PDF (${validProducts.length} products)`,
      );
    } catch (error) {
  ...
  const exportAllBarcodesPNG = async () => {
    ...
    validProducts.forEach((product, index) => {
      ...
      try {
        ...
        console.error(
          `Error exporting PNG for product ${product.name}:`,
          error,
        );
      }
    });
          ...
          {failedProducts.size > 0 && (
            <Alert className="mb-4">
              ...
              <AlertDescription className="flex items-center justify-between">
                <span>
                  {failedProducts.size} barcode
                  {failedProducts.size > 1 ? "s" : ""} failed to generate. This
                  may be due to invalid barcode data or missing product
                  information.
                </span>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={retryFailedBarcodes}
              ...
              <Table>
                ...
                <TableBody>
                  {products.map((product) => {
                    const canvas = barcodeCanvases.get(
                      Number(product.productId),
                    );
                    const hasFailed = failedProducts.has(
                      Number(product.productId),
                    );
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                exportIndividualBarcodePNG(product)
                              }
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                exportIndividualBarcodePDF(product)
                              }
                              disabled={!canvas || hasFailed}
                              title="Export PDF"
                            >
