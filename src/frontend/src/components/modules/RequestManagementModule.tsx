...
interface RequestRowProps {
  ...
  onApprove: (principalStr: string) => void;
  onReject: (principalStr: string) => void;
...
function RequestRow({
  ...
  // useGetUserProfile accepts Principal
...
export default function RequestManagementModule({
  ...
  const {
    ...
    error,
    refetch,
  } = useGetAllApprovalRequests({ refetchInterval: 30_000 });
  const approveUserMutation = useApproveUser();
  const rejectUserMutation = useRejectUser();
  const queryClient = useQueryClient();
  const [filterStatus, setFilterStatus] = useState<FilterStatus>("all");
  const [activeActionPrincipal, setActiveActionPrincipal] = useState<
    string | null
  >(null);
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {(error as Error)?.message ||
                "An error occurred while fetching approval requests."}
            </p>
          </div>
