...
function NotificationItem({
  ...
  const timeAgo = formatDistanceToNow(new Date(timestampMs), {
    addSuffix: true,
  });
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <p
                className={`text-sm font-semibold ${notification.isRead ? "text-muted-foreground" : "text-foreground"}`}
              >
                {notification.title}
              </p>
              <p
                className={`text-sm mt-0.5 ${notification.isRead ? "text-muted-foreground" : "text-foreground/80"}`}
              >
  ...
  const handleMarkAllRead = async () => {
    ...
    await Promise.all(
      unreadNotifications.map((n) =>
        markAsReadMutation.mutateAsync(n.notificationId),
      ),
    );
  };
            ...
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  You'll be notified about account approvals, rejections, and
