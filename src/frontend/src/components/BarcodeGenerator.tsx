...
export default function BarcodeGenerator({
  ...
  const [_librariesLoaded, setLibrariesLoaded] = useState({
    jsBarcode: false,
    qrCode: false,
    jsPDF: false,
  });
  ...
  const validateProduct = (): { valid: boolean; error?: string } => {
    ...
    if (product.barcode.length < 1 || product.barcode.length > 80) {
      return {
        valid: false,
        error: "Invalid barcode length (must be 1-80 characters)",
      };
    }
    return { valid: true };
  };
  ...
  const generateCode128 = async () => {
    try {
      ...
      try {
        ...
        if (
          code128CanvasRef.current.width === 0 ||
          code128CanvasRef.current.height === 0
        ) {
          setCode128Error("Failed to generate barcode image");
        }
      } catch (barcodeError: any) {
        setCode128Error(
          barcodeError.message || "Failed to generate Code128 barcode",
        );
  ...
  const generateQRCode = async () => {
    try {
      ...
      try {
        ...
        // Verify canvas has content
        if (
          qrCanvasRef.current.width === 0 ||
          qrCanvasRef.current.height === 0
        ) {
  ...
  const downloadImage = (format: "code128" | "qr") => {
    const canvas =
      format === "code128" ? code128CanvasRef.current : qrCanvasRef.current;
  ...
  const downloadPDF = async (format: "code128" | "qr") => {
    ...
    try {
      ...
      pdf.save(
        `${product.name.replace(/[^a-z0-9]/gi, "_")}-${format}-${product.barcode}.pdf`,
      );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="space-y-4">
          ...
          <Tabs
            value={activeTab}
            onValueChange={(v) => setActiveTab(v as "code128" | "qr")}
          >
