...
export default function InvoiceGenerator({
  ...
  const generateInvoiceImage = async () => {
    ...
    let metaY = 110;
    ctx.fillText(
      `Invoice No: SG-${invoice.invoiceId}`,
      canvasWidth - padding,
      metaY,
    );
    metaY += 20;
    ctx.fillText(
      `Invoice Date: ${formatDate(invoice.created)}`,
      canvasWidth - padding,
      metaY,
    );
    if (invoice.dueDate) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional — only trigger when logoLoaded changes, not on every handleGenerate reference change
  useEffect(() => {
